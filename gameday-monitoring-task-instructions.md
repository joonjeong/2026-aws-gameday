...
## 작업 2. 아키텍처 기반 모니터링 대시보드 생성

### 목표

작업 1에서 파악한 실제 리소스를 기준으로 CloudWatch Dashboard 를 생성하거나 정리한다.
...
- 위젯은 “가용성”, “지연”, “용량”, “오류”, “의존성 상태” 기준으로 배치한다.
- 애플리케이션 계층 메트릭이 아직 없으면 AWS 관리형 메트릭 중심으로 먼저 완성한다.

### 최소 포함 범주

- ALB / Target Group
- 현재 서비스 런타임 EC2 또는 ECS
- DynamoDB
...
대시보드 구성 원칙:
- 첫 줄은 서비스 전체 건강도
- 다음 줄은 진입 계층(ALB/Target Group)
- 다음 줄은 현재 실제 런타임(EC2 또는 ECS)
- 다음 줄은 데이터 저장소(DynamoDB, RDS 등)
- 마지막 줄은 향후 Application Signals / JVM / 애플리케이션 위젯 placeholder
...
- 애플리케이션 메트릭이 아직 없다면 optional 섹션으로 분리해라.
...
대시보드에서 가장 중요한 운영 리스크를 기준으로 CloudWatch Alarm 과 SNS 기반 알림 체계를 구성한다.
