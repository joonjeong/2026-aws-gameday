...
## 2026-04-06T01:54 — k6 Load 테스트 실행 및 CloudWatch 지표 분석

- **시나리오**: `load` (vus 0→50→0, 총 5분 / ramp-up 1m → steady 3m → ramp-down 1m)
- **대상**: `unicorn-rental-alb-97259582.ap-northeast-2.elb.amazonaws.com`

### CloudWatch 지표 (01:50~01:59 구간)

| 시각 | RequestCount/min | ResponseTime p99 | CPU (avg/max) | 5xx |
|---|---|---|---|---|
| 01:50 | 3,880 | 15.7ms | 6.8% / 9.5% | 0 |
| 01:51 | 3,924 | 1.8ms | — | 0 |
| 01:52 | 3,930 | 1.4ms | — | 0 |
| 01:53 | 3,753 | 1.4ms | — | 0 |
| 01:54 | 40 | 8.2s | — | 0 |
| 01:55 | 132 | 31.4s | 9.4% / 9.5% | 0 |
| 01:56 | 144 | 40.7s | — | 0 |
| 01:57 | 160 | 54.2s | — | 0 |
| 01:58 | 141 | 55.2s | — | 0 |
...
- **01:50~01:53**: 트래픽 정상 처리 (~65 req/s), p99 응답시간 1~16ms로 안정적
- **01:54 이후**: 요청 수 급감 (40건) + p99 응답시간 급등 (8s → 55s)
  - 테스트가 중단됐음에도 응답시간이 치솟은 것은 **연결 큐에 쌓인 요청이 타임아웃되며 처리된 것**으로 추정
  - 5xx는 0건 — 에러 없이 처리됨
- **CPU**: 최대 9.5% 수준 — EC2 인스턴스 여유 충분, 스케일아웃 미발생

### 결론

- EC2 기반 서비스는 ~65 req/s 부하에서 안정적으로 동작
- 부하 중단 후 응답시간 급등 패턴은 정상 (큐 드레인)
- ECS 전환 후 동일 시나리오 재실행하여 비교 필요
