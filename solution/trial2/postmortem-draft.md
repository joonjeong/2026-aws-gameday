...
- **근본 원인**: Dockerfile에 `aws` CLI 미포함 → `/actuator/health` 내부에서 `aws dynamodb describe-table` subprocess 실행 실패 → 503 반환
- **복구**: Dockerfile에 `awscli` 추가 → 재빌드/재푸시/재배포 (01:16)
- **소요 시간**: ~24분 (배포 후 감지까지 포함)
- **더 일찍 했어야 했던 것**:
  ...
  - **ECS TG Matcher**: `200-399`로 처음부터 설정했어야 함 (EC2 TG와 동일하게)
